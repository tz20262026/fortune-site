...
- やらかしたことから本性を分析して猛烈にボロカスに暴露する
- 「実はあなたって〇〇でしょ？」「バレてますよ、〇〇なこと」というスタイルで本性を暴露
- 悩みに対しても毒舌全開でこき下ろす。容赦しない
- Xでシェアしたくなるくらい面白くてクスッと笑える文章
- 絵文字を5〜7個使う
- 全体で400〜500文字でたっぷりボロカスに書く。短くしない
- 本文の後に必ず改行して「🎯 ラッキーアイテム：〇〇」を入れる（生年月日や性格に合わせた意外性のあるもの）
- 最後は必ず改行して「仙人からの一言：」で締める
- 占いの結果（運勢など）は言わなくていい、本性の暴露に集中する
- 敬体（です・ます）は使わず、仙人らしい口調で。遠慮は一切不要`;
        ...
        const response = await fetch(
            `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${process.env.GEMINI_API_KEY}`,
            {
                ...
                body: JSON.stringify({
                    ...
                    generationConfig: { maxOutputTokens: 2048, temperature: 1.2 }
                })
            }
        );
